...
$(() => {
        $("#product-search").on("submit", function (e) {
            ...
            query({ type: 'products_search', search:search }).then(showResults);
        })
    ...
    $(".js-sort").on("change", function (e) {
        (
            this.value == 1 ? query({ type: 'product_sort', column: 'date_create', dir: 'ASC' }) :
                this.value == 2 ? query({ type: 'product_sort', column: 'date_create', dir: 'DESC' }) :
